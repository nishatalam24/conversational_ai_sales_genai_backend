...
// // // Enhanced function to analyze user intent
...
// //   // Sales data keywords - more comprehensive
...
// //   if (hasSalesKeywords || (!hasProductKeywords && !hasAnalyticsKeywords)) {
...
// //     if (lowerQuery.includes('top')) parameters.insightType = 'top_products';
...
// // Try asking:
// // • "Show me California sales performance"
// // • "What are our top selling products?"
// // • "Compare furniture vs technology sales"
...
// //         aiContext = `I found sales data${parameters.state ? ` for ${parameters.state}` : ''}${parameters.category ? ` in ${parameters.category}` : ''}:
        
// // 📊 Total Sales: $${functionResult.summary.totalSales.toLocaleString()}
// // 🛒 Total Transactions: ${functionResult.summary.totalTransactions}
// // 💰 Average Transaction: $${functionResult.summary.avgTransactionValue.toLocaleString()}

// // Key insights:
// // ${functionResult.insights.join('\n')}

// // The dashboard now shows detailed charts and visualizations!`;
// //         break;
        
// //       case 'getProductInsights':
// //         functionResult = getProductInsights(parameters);
// //         aiContext = `Here are the product insights:

// // ${functionResult.insights ? functionResult.insights.join('\n') : 'Product analysis completed'}

// // ${functionResult.products ? 
// //   `Top Products:\n${functionResult.products.slice(0, 5).map(p => `• ${p.product}: $${p.sales.toLocaleString()}`).join('\n')}` : 
...
// //     let conversationText = `You are a specialized AI sales analyst. You ONLY help with sales data analysis and business performance. 
...
// // Provide a helpful, enthusiastic response about the sales data. Stay focused on business insights and recommendations. Be conversational and highlight key findings.`;
...
// // Enhanced function to analyze user intent with better location detection
// function analyzeQuery(query) {
//   const lowerQuery = query.toLowerCase();
//   console.log('Analyzing query:', query);
...
// // Enhanced function to generate contextual AI response
...
//     case 'getSalesAnalytics':
//       if (parameters.analysisType === 'location_analysis') {
//         const uniqueStates = [...new Set(functionResult.mapData?.map(item => item.state))].filter(Boolean);
//         const uniqueCities = [...new Set(functionResult.chartData?.cityBreakdown?.map(item => item.city))].filter(Boolean);
...
//     case 'getProductInsights':
//       context = `🛍️ **Product Performance Insights**

// **🏆 Top Products:**
// ${functionResult.products?.slice(0, 5).map(p => 
//   `• ${p.product}: $${p.sales.toLocaleString()}`
...
// **Try asking:**
// • "How many states are we selling in?"
// • "Show me top cities by sales"
...
// Provide a helpful, enthusiastic response about the sales data. Stay focused on business insights and recommendations. Be conversational and highlight key findings. Use the data analysis above to give specific numbers and insights.`;
...
// // Function to generate smart suggestions based on context
...
//   if (lowerQuery.includes('state') || lowerQuery.includes('states')) {
//     suggestions = [
//       "Which states have highest sales?",
...
//       `Top cities in ${parameters.state}`,
...
//       `${parameters.category} regional performance`,
