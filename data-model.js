...
// // Make sure the getSalesData function always returns a summary object
...
//       return orderDate?.includes(filters.dateRange);
//     });
//   }

//   // Calculate metrics
//   const totalSales = filteredData.reduce((sum, row) => sum + parseFloat(row.Sales || 0), 0);
...
//     const city = row.City;
//     if (!acc[city]) {
//       acc[city] = { city, sales: 0, transactions: 0, state: row.State, region: row.Region };
//     }
//     acc[city].sales += parseFloat(row.Sales || 0);
...
//     }
//     acc[category].sales += parseFloat(row.Sales || 0);
...
//     }
//     acc[date].sales += parseFloat(row.Sales || 0);
//     acc[date].transactions += 1;
//     return acc;
//   }, {});

//   // ALWAYS return a summary object, even if empty
//   const result = {
//     summary: {
//       location: filters.state || filters.city || filters.region || 'All Regions',
//       totalSales: Math.round(totalSales) || 0,
//       totalTransactions: totalTransactions || 0,
//       avgTransactionValue: Math.round(avgTransactionValue) || 0,
//       filters: filters || {}
//     },
//     chartData: {
//       cityBreakdown: Object.values(cityData).sort((a, b) => b.sales - a.sales) || [],
//       categoryBreakdown: Object.values(categoryData).sort((a, b) => b.sales - a.sales) || [],
//       timeSeries: Object.values(timeSeriesData).sort((a, b) => new Date(a.date) - new Date(b.date)) || []
//     },
//     mapData: Object.values(cityData) || [],
//     rawData: filteredData.slice(0, 50) || [],
//     insights: generateInsights(filteredData, filters) || []
//   };

//   console.log('Returning data structure:', JSON.stringify(result, null, 2));
//   return result;
// }
...
//   insights.push(`Total sales: $${totalSales.toLocaleString()}`);
//   insights.push(`Average transaction: $${avgSale.toFixed(2)}`);
  
//   // Top performing city
//   const cityPerformance = data.reduce((acc, row) => {
//     const city = row.City;
//     acc[city] = (acc[city] || 0) + parseFloat(row.Sales || 0);
//     return acc;
//   }, {});
...
//   if (topCity) {
//     insights.push(`Top performing city: ${topCity[0]} ($${topCity[1].toLocaleString()})`);
//   }
...
//     acc[product] = (acc[product] || 0) + parseFloat(row.Sales || 0);
//     return acc;
//   }, {});
...
//     insights: [`Top product: ${topProducts[0]?.product}`, `Generates $${topProducts[0]?.sales.toLocaleString()} in sales`]
//   };
// }
